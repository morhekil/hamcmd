...
test("an unconfigured key cycles through same-letter apps", function()
  local finder = newApp("Finder", "com.apple.finder")
  local telegram = newApp("Telegram", "ru.keepcoder.Telegram")
  local tablePlus = newApp("TablePlus", "com.tinyapp.TablePlus")
  local fakeHs, host = newHost({ finder, telegram, tablePlus }, finder)
  _G.hs = fakeHs

  local hamcmd = dofile("HamCmd.spoon/init.lua")
  hamcmd:start()

  host.watcherCallback(telegram:name(), fakeHs.application.watcher.activated, telegram)
  host.watcherCallback(tablePlus:name(), fakeHs.application.watcher.activated, tablePlus)

  host.hotkeys.t.callback()
  host.watcherCallback(tablePlus:name(), fakeHs.application.watcher.activated, tablePlus)
  assertEqual(host.frontmost, tablePlus, "first app")

  host.hotkeys.t.callback()
  host.watcherCallback(telegram:name(), fakeHs.application.watcher.activated, telegram)
  assertEqual(host.frontmost, telegram, "second app")

  host.hotkeys.t.callback()
  assertEqual(host.frontmost, tablePlus, "wrapped app")
end)
